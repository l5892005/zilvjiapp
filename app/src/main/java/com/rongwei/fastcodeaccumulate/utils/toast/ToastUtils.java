package com.rongwei.fastcodeaccumulate.utils.toast;
...
public final class ToastUtils {
    ...
    public static void show(CharSequence text) {
        ...
        setView(createTextView(AndroidApplication.getInstance().getApplicationContext()));
        sToast.setText(text);
    ...
    private static TextView createTextView(Context context) {
        ...
        sTextView.setId(R.id.toast_main_text_view_id);
        sTextView.setTextColor(sDefaultStyle.getTextColor());
        sTextView.setTextSize(TypedValue.COMPLEX_UNIT_PX, sp2px(context, sDefaultStyle.getTextSize()));
        sTextView.setPadding(dp2px(context, sDefaultStyle.getPaddingLeft()), dp2px(context, sDefaultStyle.getPaddingTop()),
                dp2px(context, sDefaultStyle.getPaddingRight()), dp2px(context, sDefaultStyle.getPaddingBottom()));
        sTextView.setLayoutParams(new ViewGroup.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT));
        // setBackground API版本兼容
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
            sTextView.setBackground(drawable);
        } else {
            sTextView.setBackgroundDrawable(drawable);
        }

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            sTextView.setZ(sDefaultStyle.getZ()); // 设置 Z 轴阴影
        }

        if (sDefaultStyle.getMaxLines() > 0) {
            sTextView.setMaxLines(sDefaultStyle.getMaxLines()); // 设置最大显示行数
        }
