package com.rongwei.fastcodeaccumulate.utils.photoview;
...
public class PhotoView extends AppCompatImageView {
    ...
    private Transform mTranslate = new PhotoView.Transform();
...
//            mScaleCenter.set(detector.getFocusX(), detector.getFocusY());
    ...
    private class Transform implements Runnable {
        ...
        void withClip(float fromX, float fromY, float deltaX, float deltaY, int d, PhotoView.ClipCalculate c) {
    ...
    public class START implements PhotoView.ClipCalculate {
        public float calculateTop() {
            return mImgRect.top;
        }
    }

    public class END implements PhotoView.ClipCalculate {
        public float calculateTop() {
            return mImgRect.bottom;
        }
    }

    public class OTHER implements PhotoView.ClipCalculate {
        public float calculateTop() {
    ...
    public void animaFrom(Info info) {
        if (isInit) {
            ...
            if (info.mWidgetRect.width() < info.mImgRect.width() || info.mWidgetRect.height() < info.mImgRect.height()) {
                ...
                PhotoView.ClipCalculate c = info.mScaleType == ImageView.ScaleType.FIT_START ? new PhotoView.START() : info.mScaleType == ImageView.ScaleType.FIT_END ? new PhotoView.END() : new PhotoView.OTHER();
    ...
    public void animaTo(Info info, Runnable completeCallBack) {
        if (isInit) {
            ...
            if (info.mWidgetRect.width() < info.mRect.width() || info.mWidgetRect.height() < info.mRect.height()) {
                ...
                final PhotoView.ClipCalculate c = info.mScaleType == ImageView.ScaleType.FIT_START ? new PhotoView.START() : info.mScaleType == ImageView.ScaleType.FIT_END ? new PhotoView.END() : new PhotoView.OTHER();
